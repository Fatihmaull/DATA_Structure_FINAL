...
public class PenerjemahKodeMorse {
    ...
    public void createAndShowGUI() {
        JFrame frame = new JFrame("Penerjemah Kode Morse");
        ...
        inputArea.setBorder(BorderFactory.createTitledBorder("Input Teks / Kode Morse"));
    ...
    private static class MorseTree {
        ...
        private void addMorseCode(char letter, String code) {
            ...
            for (char c : code.toCharArray()) {
                if (c == '.') {
                    if (current.dot == null)
                        current.dot = new MorseNode('\0');
                    current = current.dot;
                } else if (c == '-') {
                    if (current.dash == null)
                        current.dash = new MorseNode('\0');
        ...
        public String encode(String text) {
            ...
            for (char c : text.toUpperCase().toCharArray()) {
                if (c == ' ')
                    ...
                    morse.append(encodeCharacter(c, root, "")).append(" ");
            }
        ...
        public String decode(String morse) {
            ...
            for (String code : morse.split(" ")) {
                if (code.equals("/"))
                    ...
                    text.append(decodeCharacter(code, root));
            }
    ...
    private static class MorsePlayer {
        ...
        public void playMorse(String morse) {
            for(char c:morse.toCharArray()){switch(c){case'.'->playTone(800,200);case'-'->playTone(800,600);case' '->sleep(200);case'/'->sleep(600);default->System.err.println("Karakter Morse tidak valid: "+c);}}
        }
